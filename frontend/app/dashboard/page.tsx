...
import { useProjects } from "@/lib/useFirestore"
import {
                  ...
                  {projectsLoading ? (
                    Array.from({ length: 3 }).map((_, i) => (
                      <tr key={i} className="border-b border-white/5">
                        <td className="px-4 py-5"><div className="h-4 w-40 skeleton rounded-lg" /></td>
                        <td className="px-4 py-5"><div className="h-4 w-20 skeleton rounded-lg" /></td>
                        <td className="px-4 py-5"><div className="h-4 w-16 skeleton rounded-lg" /></td>
                        <td className="px-4 py-5"><div className="h-4 w-20 skeleton rounded-lg" /></td>
                        <td className="px-4 py-5 text-right"><div className="h-4 w-24 skeleton rounded-lg ml-auto" /></td>
                      </tr>
                    ))
                  ) : projects.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-12 text-center">
                        ...
                        <a href="/roof-detection" className="mt-3 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-neon-blue text-background text-xs font-black uppercase tracking-wider hover:bg-neon-blue/90 transition-all glow-blue">
                          <Upload className="w-3.5 h-3.5" /> Start First Analysis
                        </a>
                      </td>
                    </tr>
                  ) : (
                    projects.map((p, i) => {
                      const displayStatus = firestoreStatusMap[p.status] || "Pending"
                      const panels = p.panel_count ? `${p.panel_count} panels` : p.estimated_capacity_kw ? `${p.estimated_capacity_kw.toFixed(1)} kW` : "—"
                      const savings = p.estimated_annual_savings
                        ? `$${Math.round(p.estimated_annual_savings).toLocaleString()}/yr`
                        : "—"
                      return (
                        <motion.tr
                          key={p.id}
                        ...
                        >
                          <td className="px-4 py-5">
                            <div className="flex flex-col">
                              <span className="text-sm font-bold text-foreground group-hover:text-neon-blue transition-colors">
                          ...
                          <td className="px-4 py-5">
                            <span className={cn("text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-lg border", statusColors[displayStatus] || statusColors["Pending"])}>
                              {displayStatus}
                            </span>
                          </td>
                          <td className="px-4 py-5 text-xs font-mono text-muted-foreground">{panels}</td>
                          <td className="px-4 py-5 text-sm font-black text-neon-green tracking-tight">{savings}</td>
                          <td className="px-4 py-5 text-right text-xs font-semibold text-muted-foreground">
                            {formatProjectDate(p.updated_at || p.created_at)}
                          </td>
                        </motion.tr>
                      )
                    })
